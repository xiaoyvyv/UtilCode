...
public class MapUtils {
     ...
     */
    @SafeVarargs
    public static <K, V> Map<K, V> newUnmodifiableMap(final PairUtils<K, V>... pairs) {
        return Collections.unmodifiableMap(newHashMap(pairs));
    }

    @SafeVarargs
    public static <K, V> HashMap<K, V> newHashMap(final PairUtils<K, V>... pairs) {
        HashMap<K, V> map = new HashMap<>();
        if (pairs == null || pairs.length == 0) {
            return map;
        }
        for (PairUtils<K, V> pair : pairs) {
    ...
    @SafeVarargs
    public static <K, V> LinkedHashMap<K, V> newLinkedHashMap(final PairUtils<K, V>... pairs) {
    ...
    @SafeVarargs
    public static <K, V> Hashtable<K, V> newHashTable(final PairUtils<K, V>... pairs) {
        ...
        try {
            ...
            forAllDo(map, new Closure<K1, V1>() {
                ...
                public void execute(K1 key, V1 value) {
                    PairUtils<K2, V2> pair = transformer.transform(key, value);
    ...
    public interface Transformer<K1, V1, K2, V2> {
        PairUtils<K2, V2> transform(K1 k1, V1 v1);
    }
}
