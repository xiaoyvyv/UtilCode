...
import com.xiaoyv.annotation.IntRange;
...
public final class ThreadUtils {
    ...
    public static void cancel(ExecutorService executorService) {
        if (executorService instanceof ThreadPoolExecutor4Util) {
            ...
            System.err.println("ThreadUtils：The executorService is not ThreadUtils's pool.");
        }
    }
    ...
    private static <T> void execute(final ExecutorService pool, final Task<T> task,
        ...
        synchronized (TASK_POOL_MAP) {
            if (TASK_POOL_MAP.get(task) != null) {
                System.err.println("ThreadUtils：Task can only be executed once.");
                return;
            }
    ...
    private static Executor getGlobalDeliver() {
        if (sDeliver == null) {
            sDeliver = ThreadUtils::runOnMainThread;
        }
        return sDeliver;
    }
